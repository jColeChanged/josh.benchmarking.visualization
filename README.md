# josh.benchmarking.visualization
...
* [`src/josh\benchmarking\visualization/`](src/josh\benchmarking\visualization/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/josh\benchmarking\visualization/core.cljs): contains the SPA entry point, `init`
* [`test/josh\benchmarking\visualization/`](test/josh\benchmarking\visualization/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
5. Clone this repo and open a terminal in the `josh.benchmarking.visualization` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/josh\benchmarking\visualization/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns josh.benchmarking.visualization.example
  (:require [josh.benchmarking.visualization.config :as config])
